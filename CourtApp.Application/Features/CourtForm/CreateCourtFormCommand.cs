...
{
    public class CreateCourtFormCommand : IRequest<Result<string>>
    {
    ...
    public class CreateCourtFormCommandHandler : IRequestHandler<CreateCourtFormCommand, Result<string>>
    {
        ...
        public async Task<Result<string>> Handle(CreateCourtFormCommand request, CancellationToken cancellationToken)
        {
            ...
            if (formExists)
                return await Result<string>.FailAsync("The provided details already exist");

            var formEntity = mapper.Map<CourtFormTypeEntity>(request);
            await repository.InsertAsync(formEntity);
            await unitOfWork.Commit(cancellationToken);

            return await Result<string>.SuccessAsync("Record created successfull!");
