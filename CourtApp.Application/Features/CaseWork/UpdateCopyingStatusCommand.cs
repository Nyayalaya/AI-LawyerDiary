...
{
    public class UpdateCopyingStatusCommand : IRequest<Result<string>>
    {
        public List<Guid> CaseId { get; set; }
        public int Status { get; set; }
    }
    public class UpdateCopyingStatusCommandHandler : IRequestHandler<UpdateCopyingStatusCommand, Result<string>>
    {
        ...
        public async Task<Result<string>> Handle(UpdateCopyingStatusCommand request, CancellationToken cancellationToken)
        {
            ...
            if(latestEntities.Count==0)
                return await Result<string>.FailAsync($"The proceedings of cases is not found!");
            ...
            return await Result<string>.SuccessAsync($"Case copying receive status update successfull!");
        }
    }
}
